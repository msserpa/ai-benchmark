...
int main(int argc, char ** argv) {
	...
	for (frame_num = 0; frame_num <= max_frame_num; frame_num++) {
		...
		MAT *img_dilated = dilate_f(max_gicov);
        gettimeofday(&tv, NULL); 
		...
		for(i = 0; i < max_gicov->m; i++) {
			for(j = 0; j < max_gicov->n; j++) {
				if(!double_eq(m_get_val(max_gicov,i,j), 0.0) && double_eq(m_get_val(img_dilated,i,j), m_get_val(max_gicov,i,j))) {
					crow[pair_counter] = i;
					ccol[pair_counter] = j;
					pair_counter++;
				}

...
__global__ void ellipsematching_kernel(float * grad_x, float * grad_y, int grad_m, float * sin_angle, float * cos_angle, int * tX, int * tY, float * gicov) {
	float Grad[NPOINTS];
...
float *ellipsematching_setup(int grad_m, int grad_n, float *host_grad_x, float *host_grad_y) {
  ...
  // execute the kernel
  ellipsematching_kernel<<< grid, threads, 0 >>>(device_grad_x, device_grad_y, grad_m, NULL, NULL, NULL, NULL, device_gicov);

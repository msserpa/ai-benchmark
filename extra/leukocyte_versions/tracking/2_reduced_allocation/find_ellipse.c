...
void compute_constants() {
	...
	int n, k;
	// Compute sin_angle and cos_angle
...
MAT *ellipsematching(MAT *grad_x, MAT *grad_y) {
	...
	// compute result
	float *host_gicov = ellipsematching_setup(grad_m, grad_n, host_grad_x, host_grad_y);

	// copy results into a new matrix
...
MAT *dilate_f(MAT *img_in) {
	...
	float *host_img_dilated = dilate_f_setup(max_gicov_m, max_gicov_n, strel_m, strel_n);

	// copy results into new matrices

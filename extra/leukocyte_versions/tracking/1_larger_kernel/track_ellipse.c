...
MAT *MGVF(MAT *I, double vx, double vy) {
	...
	*/
	...
	MAT *IMGVF = m_get(m, n), *IMGVF_old = m_get(m, n);
	for (i = 0; i < m; i++) {
		for (j = 0; j < n; j++) {
			m_set_val(IMGVF_old, i, j, m_get_val(I, i, j));
		}
	}
	...
	while ((iter < iterations) && (mean_diff > converge)) {
		...
		heaviside_cuda( UHe,  U, -vy,      epsilon);
		heaviside_cuda( DHe,  D,  vy,      epsilon);
		heaviside_cuda( LHe,  L, -vx,      epsilon);
		heaviside_cuda( RHe,  R,  vx,      epsilon);
		heaviside_cuda(URHe, UR,  vx - vy, epsilon);
		heaviside_cuda(DRHe, DR,  vx + vy, epsilon);
		heaviside_cuda(ULHe, UL, -vx - vy, epsilon);
		heaviside_cuda(DLHe, DL,  vy - vx, epsilon);
		...
		*/
		...
		IMGVF_cuda(IMGVF_old, IMGVF, vx, vy, epsilon);
		
		// Precompute constant to avoid division in the for loops below
		double one_over_lambda = 1.0 / lambda;
		...
		for (i = 0; i < m; i++) {
			for (j = 0; j < n; j++) {
				// Compute IMGVF -= (1 / lambda)(I .* (IMGVF - I))
				double vI = m_get_val(I, i, j);
				double cur_val = m_get_val(IMGVF, i, j);
				double new_val = cur_val - (one_over_lambda * vI * (cur_val - vI));
				m_set_val(IMGVF, i, j, new_val);
				
				// Keep track of the absolute value of the differences
				//  between this iteration and the previous one
				total_diff += fabs(new_val - m_get_val(IMGVF_old, i, j));
			}
		}
	...
	//printf("heaviside: %.5f seconds, %d iterations\n", ((float) total_time) / (1000*1000), iter);

...
int main(int argc, char ** argv) {
	...
	// Open video file
	avi_t *cell_file = AVI_open_input_file("testfiles/testfile.avi", 1);
	if (cell_file == NULL)	{
	...
	printf("ellipsematching: %.5f seconds\n", ((float) (end_time - start_time)) / (1000*1000));
	...
	for (n = 0; n < x_result_len; n++) {
		if ((G[n] < -1 * threshold) || G[n] > threshold) {
			...
			if ((m_min(x) > b) && (m_min(y) > b) && (m_max(x) < cell_file->width - b) && (m_max(y) < cell_file->height - b)) {
				...
				QAY_CENTERS[k_count] = mean(Y) + TOP; // + 40.0; Matlab code does not have the plus 40
	...
	// Report total detection time
	gettimeofday(&tv, NULL); 
	long long detection_end_time = tv.tv_sec*1000000 + tv.tv_usec;
	printf("Detection: %.5f seconds\n", ((float) (detection_end_time - program_start_time)) / (1000*1000));
	printf("K (num cells detected): %d\n", k_count);
	
	// For testing, use the same QA*_CENTERS as the Matlab version
	int k_count_Matlab = 36;
	double QAX_Matlab[] = {82.8482,96.6844,104.3019,104.6975,113.8853,
						   137.5783,170.1458,189.9226,193.3009,212.6460,
						   220.8008,225.7166,236.1537,248.3190,253.9510,
						   282.0804,293.6349,319.1482,322.6878,336.3909,
						   340.4191,353.8329,371.1758,388.0309,408.5930,
						   462.0398,482.5480,486.3902,488.6104,499.0176,
						   551.1538,560.2374,561.5644,580.3312,618.9730,
						   618.8302};
	double QAY_Matlab[] = {224.2819,129.0785,148.2362,184.4784,227.5777,
						   177.8867,199.2778,189.7474,261.5210,177.1257,
						   217.6680,138.9823,254.6346,262.8605,129.0428,
						   177.9658,140.4669,133.1677,167.5652,215.1404,
						   189.5528,129.5083,207.7972,183.7695,133.3875,
						   147.8411,226.6543,171.0379,153.8237,181.2465,
						   222.6139,200.0578,170.8748,209.6422,165.7423,
						   192.7234};
	for (i = 0; i < k_count_Matlab; i++) {
		// The Matlab values need to be decremented by one
		QAX_Matlab[i] -= 1.0;
		QAY_Matlab[i] -= 1.0;
	}
	...
	gettimeofday(&tv, NULL); 
	start_time = tv.tv_sec * 1000000 + tv.tv_usec;
	//ellipsetrack(cell_file, QAX_CENTERS, QAY_CENTERS, k_count, radius, 1);
	ellipsetrack(cell_file, QAX_Matlab, QAY_Matlab, k_count_Matlab, 10.0, num_frames);
	gettimeofday(&tv, NULL); 
	end_time = tv.tv_sec * 1000000 + tv.tv_usec;
	printf("ellipsetrack: %.5f seconds\n", ((float) (end_time - start_time)) / (float) (1000*1000*num_frames));
	// printf("%.5f\n", ((float) (end_time - start_time)) / (float) (1000*1000*num_frames));

...
__global__ void GICOV_kernel(int height, int width, int cropped_height, int cropped_width, float *gicov) {
	int i, j, k, n, x, y;
	
	// Determine this thread's pixel
	int thread_id = (blockIdx.x * blockDim.x) + threadIdx.x;
	i = (thread_id / cropped_height) + MAX_RAD + 2;
	j = (thread_id % cropped_height) + MAX_RAD + 2;
	
	// Make sure the thread has not gone off the edge of the cropped image
	if (i < (cropped_width + MAX_RAD + 2)) {
		...
		gicov[(i * height) + j] = max_GICOV;
	}
}


// Sets up and invokes the GICOV kernel and returns its output
float *GICOV_CUDA(int image_height, int image_width, float *host_grad_x, float *host_grad_y) {

	int MaxR = MAX_RAD + 2;

	// Allocate device memory
	unsigned int grad_mem_size = sizeof(float) * image_height * image_width;
	...
	int cropped_height = image_height - (2 * MaxR);
	int cropped_width = image_width - (2 * MaxR);
	int num_threads = cropped_height * cropped_width;
	int threads_per_block = 128;
	int num_blocks = (num_threads + threads_per_block - 1) / threads_per_block;
    
	// Execute the GICOV kernel
	GICOV_kernel <<< num_blocks, threads_per_block >>> (image_height, image_width, cropped_height, cropped_width, device_gicov);
...
__global__ void dilate_kernel(int img_m, int img_n, int strel_m, int strel_n, float *dilated) {	
	...
	// Make sure the thread has not gone off the edge of the matrix
	if (j < img_n) {

...
__global__ void IMGVF_kernel(float *IMGVF_in, float *IMGVF_out, float *I, float *partial_sums,
                             int *converged, float vx, float vy, float e, int m, int n,
							 int num_blocks, float converge, int *in_out) {
	
	// If we already know that we have converged then return
	...
	__shared__ int local_converged;
	if (threadIdx.x == 0) {
		int k;
		float sum = 0.0;
		...
		if (mean < converge) {
			if (blockIdx.x == 0) *converged = 1;
			local_converged = 1;
		} else local_converged = 0;
	}
	__syncthreads();
	
	// If we just figured out that we have converged then return
	if (local_converged == 1) return;
	...
	if (threadIdx.x == 0) partial_sums[blockIdx.x] = val[0];
}
	...
	while ((iter < iterations) && (! host_converged)) {
		...
		for (i = 0; i < unroll; i++) {
			IMGVF_kernel <<< num_blocks, threads_per_block >>> (device_IMGVF_in, device_IMGVF_out, device_I,
																device_partial_sums, device_converged,
																(float) vx, (float) vy, (float) e, m, n,
																num_blocks, cutoff, device_in_out);
			
			IMGVF_kernel <<< num_blocks, threads_per_block >>> (device_IMGVF_out, device_IMGVF_in, device_I,
																device_partial_sums, device_converged,
																(float) vx, (float) vy, (float) e, m, n,
																num_blocks, cutoff, device_in_out);
		}

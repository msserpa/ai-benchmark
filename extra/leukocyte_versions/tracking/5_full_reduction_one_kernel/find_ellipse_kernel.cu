...
__global__ void ellipsematching_kernel(float * grad_x, float * grad_y, int grad_m, int grad_n, float * gicov) {
	...
	for (k = 0; k < NCIRC; k++) {
		...
		for (n = 0; n < NPOINTS; n++) {
			...
			/*
            float p =   grad_x[x * grad_m + y] * c_cos_angle[n] + 
                        grad_y[x * grad_m + y] * c_sin_angle[n];
...
float *ellipsematching_setup(int grad_m, int grad_n, float *host_grad_x, float *host_grad_y) {
	// Note: kernel execution is approximately two orders of
	//  magnitude slower than memory copying for this kernel

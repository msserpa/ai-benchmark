...
void init_bucketsort(int listsize)
{
    ...
    CUDA_SAFE_CALL(cudaMalloc((void**) &d_offsets, histosize * sizeof(unsigned int)));
	pivotPoints = (float *)malloc(DIVISIONS * sizeof(float)); 

    CUDA_SAFE_CALL(cudaMalloc((void**) &d_indice, listsize * sizeof(int)));
	historesult = (float *)malloc(histosize * sizeof(float)); 

	CUDA_SAFE_CALL(cudaMalloc((void**) &l_pivotpoints, DIVISIONS * sizeof(float))); 
	CUDA_SAFE_CALL(cudaMalloc((void**) &l_offsets, DIVISIONS * sizeof(int))); 

	int blocks = ((listsize - 1) / (BUCKET_THREAD_N * BUCKET_BAND)) + 1; 
	CUDA_SAFE_CALL(cudaMalloc((void**) &d_prefixoffsets, blocks * BUCKET_BLOCK_MEMORY * sizeof(int))); 
...
{
    CUDA_SAFE_CALL(cudaFree(d_indice));
	CUDA_SAFE_CALL(cudaFree(d_offsets));
	CUDA_SAFE_CALL(cudaFree(l_pivotpoints));
	CUDA_SAFE_CALL(cudaFree(l_offsets)); 
	free(pivotPoints); 
	free(h_offsets);
	free(historesult);	
	CUDA_SAFE_CALL(cudaFree(d_prefixoffsets)); 
	closeHistogram1024();
}
				...
				unsigned int *origOffsets)
{
	...
	////////////////////////////////////////////////////////////////////////////
	CUDA_SAFE_CALL(cudaMemset((void *) d_offsets, 0, histosize * sizeof(int))); 
	...
	///////////////////////////////////////////////////////////////////////////
	CUDA_SAFE_CALL(cudaMemcpy(l_pivotpoints, pivotPoints, (DIVISIONS)*sizeof(int), cudaMemcpyHostToDevice)); 
	CUDA_SAFE_CALL(cudaMemset((void *) d_offsets, 0, DIVISIONS * sizeof(int))); 
	CUDA_SAFE_CALL(cudaBindTexture(0, texPivot, l_pivotpoints, DIVISIONS * sizeof(int))); 
    ...
    CUT_CHECK_ERROR("Bucketsorting failed");
}

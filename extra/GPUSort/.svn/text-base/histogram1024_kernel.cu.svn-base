...
void initHistogram1024(void){
    CUDA_SAFE_CALL( cudaMalloc((void **)&d_Result1024, HISTOGRAM_SIZE ));
}

//Internal memory deallocation
void closeHistogram1024(void){
    CUDA_SAFE_CALL( cudaFree(d_Result1024) );
}
...
void histogram1024GPU(
    ...
    int dataN)
{
    CUDA_SAFE_CALL( cudaMemset(d_Result1024, 0, HISTOGRAM_SIZE) );
    ...
    CUT_CHECK_ERROR("histogram1024Kernel() execution failed\n");
    CUDA_SAFE_CALL( cudaMemcpy(h_Result, d_Result1024, HISTOGRAM_SIZE, cudaMemcpyDeviceToHost) );
}

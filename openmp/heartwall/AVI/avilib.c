  ...
  /* Read first 12 bytes and check that this is an AVI file */
   ...
   if(AVI->idx)
   {
      ...
      else
      {
         ...
         if(avi_read(AVI->fdes,data,8)!=8) ERR_EXIT(AVI_ERR_READ)
         if( (long) strncasecmp((char *)data,(char *)AVI->idx[i],4)==0 && (long) str2ulong((unsigned char *)data+4)==len )
         {
   ...
   while(1)
   {
      ...
      if(strncasecmp(data,AVI->video_tag,3) == 0)
      {
         *len = n;
         ...
         if((long)avi_read(AVI->fdes,vidbuf,n) != n ) return 0;
         return 1;
      }
      else if(strncasecmp(data,AVI->track[AVI->aptr].audio_tag,4) == 0)
      {
         *len = n;
         ...
         if((long)avi_read(AVI->fdes,audbuf,n) != n ) return 0;
         return 2;
         break;
      }
      else
